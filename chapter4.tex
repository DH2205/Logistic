...
We built LogiShop in stages, developing it from a basic data-driven web application into a shipping management tool.

The stack combines a Next.js and React frontend, TypeScript, route-handler backend logic in the same project, Supabase, and PostgreSQL. Keeping the frontend and backend in one project made the system easier to develop and maintain.

The first goal was to build the core API and database. We began with endpoints for authentication, order retrieval, and route access, then designed the database schema. Once the backend was in place, we built the mapping module, which became the main way for users to view shipping routes and identify disruptions.

We then added disruption handling. Users log disruption events through the interface, and the system converts them into geographic impact zones. When a route is blocked, the platform uses graph-based shortest-path logic to suggest alternatives automatically. This makes LogiShop a planning tool rather than only a tracking tool.

\subsection{Code Implementation}

\subsubsection{Project Structure}

The frontend is organized into modules for readability. Shared UI components (layout elements, maps, and tracking) are grouped together in component folders. React context providers, such as the authentication context, manage state across pages. Screens follow the Next.js App Router: every route has its own folder with a page.tsx file, and layouts are added where needed.
...
The create order form lets users enter shipment information. Validation rules keep the stored data consistent and give operators accurate information for delivery processing.

\subsection{Data Synchronization and Fresh Tracking Context}

One of the main challenges in international shipping is tracking orders across several systems. Operators handle large amounts of data and often check multiple carrier portals by hand. LogiShop addresses this with a synchronization workflow that keeps tracking data fresher and more consistent.
...
International shipments are exposed to disruptions such as war zones, natural disasters, and extreme weather. Most tracking systems do not suggest an alternative route when these problems occur.
...
The map helps users see how packages move between countries. In simulated disruption scenarios, the reroute function identified blocked zones and generated feasible alternatives.

\subsection{Security Measures in Operational Use}

LogiShop applies security controls at the API level. Rate limiting guards against abuse and brute-force attempts, and user input is validated and sanitized before it reaches the database. During testing, the rate limit blocked excessive requests.
...
One objective of LogiShop is to reduce the operational workload on SMEs. To measure this, we compared the number of user interactions and the total time needed to complete a standard shipment monitoring task.
...
LogiShop reduces manual interactions by 83\% and task completion time by 93\%. It also performs disruption evaluation automatically, which previously required manual steps.
...
The alternative route avoids the Persian Gulf by turning south past Sri Lanka and along the East African corridor, then passing through the Suez Canal and across the Atlantic to the United States. This shows how LogiShop supports cost-impact analysis. Carrier systems mainly enforce fixed rules and provide API-based tracking updates, while LogiShop also offers disruption-aware routing to help users make decisions.

For many SMEs, the largest financial risk comes from opportunity costs and contractual penalties when deliveries arrive late. Delays can lead to invoice fines, halted operations, extra fees, and loss of customer confidence.
...
The disruption scenario costs about 18.51\% more than the baseline. The current system focuses on route optimization, so this figure serves as a decision-support estimate until real-time carrier cost data is integrated.

\subsection{Experimental Evaluation of the Multi-Criteria Decision Analysis (MCDA) Model}

Section 2.9 defined a Multi-Criteria Decision Analysis (MCDA) framework that uses weighted criteria ($w_j$) for cost and time to make routing decisions under trade-offs. In the current release, the user interface fixes the weights at a balanced setting ($w_{\text{cost}} = 0.5$, $w_{\text{time}} = 0.5$) to keep the initial MVP testing simple.

To test whether the backend routing engine still behaves correctly under different weights, we ran an offline simulation. The optimization algorithm was tested in two scenarios to see how different user priorities affect the alternative routes it generates when a disruption zone is active.

\subsubsection{Simulation Scenario Profiles}
We simulated a fixed shipment order from SGN to JFK with the disruption zone marked active, and tested two user profiles:
\begin{itemize}
    \item \textbf{Profile A (Urgent Delivery Focus):} A scenario where delivery time is critical, such as medical supplies or high-value electronics. The weights were set to $w_{\text{time}} = 0.8$ and $w_{\text{cost}} = 0.2$.
    \item \textbf{Profile B (Economic Efficiency Focus):} A scenario where budget matters most and delays are acceptable, such as bulk non-perishable goods. The weights were set to $w_{\text{time}} = 0.2$ and $w_{\text{cost}} = 0.8$.
\end{itemize}

\subsubsection{Simulation Results and Discussion}
Table~\ref{tab:mcda-simulation-results} shows how the optimization algorithm responded to these weights.
...
\midrule
\textbf{Profile A} (Urgent) & $(0.2, 0.8)$ & \$1,450 & 28 Hours & Avoids the active disruption zone using an express air corridor and accepts higher freight surcharges. \\
\addlinespace
\textbf{Profile B} (Economy) & $(0.8, 0.2)$ & \$820 & 74 Hours & Selects a multimodal sea-and-rail path. It crosses the edge of the zone and absorbs the penalty cost to keep total spending low. \\
\bottomrule
\end{tabularx}
\end{table}

The results show that the backend engine adjusts its graph edge penalties according to user priorities. When $w_{\text{time}}$ dominates, the system heavily penalizes paths that cross disruption zones because of the high accumulated \texttt{ancillary\_delay\_cost}. When $w_{\text{cost}}$ dominates, the system accepts longer transit times to reduce transport costs.

The weight-adjustment sliders are planned for the version 2.0 frontend, but this experiment confirms that the mathematical model from Chapter 2 already works in the backend.
...
We tested geocoding accuracy by running 50 location strings across six input categories. The system correctly resolved every country name, ISO-2 code, and UPS-style location format, reaching 100\% accuracy in those categories. Accuracy dropped for bare city names, which shows that the lookup depends on carrier-style country formats rather than city names alone. Invalid strings were rejected and returned null. Overall, the system correctly geocoded 70\% of the test cases.

\subsection{Security}

Security is a core requirement for a web application that manages shipment records, customer details, order histories, and tracking data. Security testing matters because added complexity increases the risk of exposing sensitive data \cite{jaiswal2014security}.
